...
describe('AddPresenterSchema', () => {
  ...
  it('should reject empty nickname', () => {
    expect(() => AddPresenterSchema.parse({ gameId: validGameId, nickname: '' })).toThrow(
      'ニックネームを入力してください'
    );
  });
...
describe('UpdateEpisodeSchema', () => {
  ...
  it('should reject when neither text nor isLie is provided', () => {
    expect(() => UpdateEpisodeSchema.parse({ episodeId: validEpisodeId })).toThrow(
  ...
  it('should reject text longer than 1000 chars', () => {
    const text = 'a'.repeat(1001);
    expect(() => UpdateEpisodeSchema.parse({ episodeId: validEpisodeId, text })).toThrow();
  });
});
...
describe('UpdateGameSchema', () => {
  ...
  it('should reject invalid player limit', () => {
    expect(() => UpdateGameSchema.parse({ gameId: validGameId, playerLimit: 0 })).toThrow();
  });
});
