  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6">
      <h2 className="mb-4 text-lg font-semibold text-gray-900">
        エピソードを追加
        {presenterNickname && (
          <span className="ml-2 text-sm font-normal text-gray-600">- {presenterNickname}</span>
        )}
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <div className="flex items-center">
            <input
              ...
              name="isLie"
              value="true"
              disabled={isSubmitting || isSuccess || !canAddMore || hasLieEpisode}
              checked={isLie}
            ...
            />
            <label htmlFor="isLie" className="ml-2 text-sm font-medium text-gray-700">
              これはウソのエピソードです
            </label>
          </div>
        ...
        <button
          type="submit"
          disabled={isSubmitting || isSuccess || !canAddMore || isOverLimit || characterCount === 0}
      ...
      <div className="mt-4 text-sm text-gray-600">
        <p>
          ...
          {!hasLieEpisode && 'ウソのエピソードを1つ含める必要があります。'}
        </p>
      </div>
    </div>
  );
