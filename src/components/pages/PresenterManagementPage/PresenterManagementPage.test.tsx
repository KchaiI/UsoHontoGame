...
vi.mock('@/components/pages/PresenterManagementPage/hooks/usePresenterManagementPage', () => ({
...
describe('PresenterManagementPage', () => {
  ...
  it('should render without crashing', () => {
    ...
    expect(screen.getByRole('heading', { name: 'プレゼンター管理' })).toBeInTheDocument();
  });
  ...
  it('should not show main content when loading', () => {
    ...
    render(<PresenterManagementPage gameId="test-game-id" />);
    expect(screen.queryByRole('heading', { name: 'プレゼンター管理' })).not.toBeInTheDocument();
  });
  ...
  it('should display page description', () => {
    mockUsePresenterManagementPage.mockReturnValue(mockUsePresenterManagementPageReturn());
    render(<PresenterManagementPage gameId="test-game-id" />);
    expect(screen.getByText(/プレゼンターを追加し、それぞれに3つのエピソード/)).toBeInTheDocument();
  });
});
