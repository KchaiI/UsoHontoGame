...
test.describe('Presenter Management Flow', () => {
  ...
  test('should add a presenter successfully', async ({ page }) => {
    ...
    await expect(page.locator('text=プレゼンターを追加しました')).toBeVisible();

    // Should appear in presenter list
    await expect(page.locator('text=田中太郎')).toBeVisible();
  });

  test('should show validation error for empty nickname', async ({ page }) => {
    await page.goto(`/games/${gameId}/presenters`);
  ...
  test('should add episode to presenter', async ({ page }) => {
    ...
    await page.click('form:has(input[name="nickname"]) button[type="submit"]');
    await expect(page.locator('text=プレゼンターを追加しました')).toBeVisible();
    ...
    // Submit episode
    await page.click('form:has(textarea[name="text"]) button[type="submit"]');

    // Should show success message
    await expect(page.locator('text=エピソードを追加しました')).toBeVisible();

    // Episode should appear in the list
    await expect(page.locator('text=私は昨日富士山に登りました。')).toBeVisible();
  });
  ...
  test('should mark episode as lie', async ({ page }) => {
    ...
    await page.fill('textarea[name="text"]', '私は宇宙飛行士の資格を持っています。');
  ...
  test('should prevent adding more than 3 episodes', async ({ page }) => {
    ...
    await expect(page.locator('text=既に3つのエピソードが登録されています')).toBeVisible();
  ...
  test('should prevent adding second lie episode', async ({ page }) => {
    ...
    await expect(page.locator('text=既にウソのエピソードが登録されています')).toBeVisible();
  });

  test('should remove presenter and cascade delete episodes', async ({ page }) => {
    await page.goto(`/games/${gameId}/presenters`);
  ...
  test('should allow adding up to 10 presenters', async ({ page }) => {
    ...
    // Should show validation error
    await expect(page.locator('text=maximum of 10 presenters')).toBeVisible();
  });
  ...
  test('should handle missing session gracefully', async ({ page }) => {
    ...
    await expect(page.locator('text=セッションが見つかりません')).toBeVisible();
  });
});
