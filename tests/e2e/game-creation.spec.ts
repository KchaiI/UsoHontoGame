...
test.describe('Game Creation Flow', () => {
  ...
  test('should show validation error for player limit below 1', async ({ page }) => {
    ...
    await expect(page.locator('text=プレイヤー数は1以上でなければなりません')).toBeVisible();

    // Should NOT redirect
    await expect(page).toHaveURL('/games/create');
  });

  test('should show validation error for player limit above 100', async ({ page }) => {
    await page.goto('/games/create');
    ...
    // Should show validation error
    await expect(page.locator('text=プレイヤー数は100以下でなければなりません')).toBeVisible();

    // Should NOT redirect
    await expect(page).toHaveURL('/games/create');
  });

  test('should show validation error for negative player limit', async ({ page }) => {
    await page.goto('/games/create');
